...
class ClassMS():
    ...
    def LoadDDFBeam(self):
        if self.GD["Beam"]["BeamModel"] == "FITS":
            ...
            stop
            from DDFacet.Data.ClassATCABeam import ClassNenufarBeam as ClassDDFBeam
    ...
    def GiveBeam(self,time,ra,dec):
        ...
        if self.GD["Beam"]["BeamModel"] == "LOFAR":
            ...
            for i in range(ra.shape[0]):
                ...
                # Beam.fill(0)
                # Beam[...,0,0]=(time-self.F_times[0])/3600
                # Beam[...,1,1]=(time-self.F_times[0])/3600
    ...
    def ReadData(self,t0=0,t1=-1,DoPrint=False,ReadWeight=False):
        ...
        log.print("[%s] Reading rows [%i -> %i]"%(self.MSName,self.ROW0,self.ROW1))
        ...
        self.F_TimeInterVal=self.TimeInterVal=table_all.getcol("INTERVAL")

        if self.ReOrder:
            log.print("  data = %s"%(self.ColName))
            vis_all=table_all.getcol(self.ColName,row0,nRowRead)[:,self.ChanSlice,:]
            if self.SubColName is not None:
                log.print("  data-= %s"%(self.SubColName))
                vis_all1=table_all.getcol(self.SubColName,row0,nRowRead)[:,self.ChanSlice,:]
                vis_all-=vis_all1
                del vis_all1
        ...
        if self.GD is not None:
            WeightInCol=self.GD["Weighting"]["WeightInCol"]
            if WeightInCol is not None and WeightInCol!="" and WeightInCol in table_all.colnames():
                ...
                flag_all[fw<MedW*self.GD["Weighting"]["WeightFlagTh"]]=1
    ...
    def ToOrigFreqOrder(self,data):
        ...
        if self.NPolOrig==2 and len(dOut.shape)>2 and dOut.shape[2]==4:
            dOut=dOut[:,:,0::3]

        return dOut[:,self.ChanSlice,...]
    ...
    def ReadMSInfo(self,MSname,DoPrint=True):
        ...
        self.tMean=np.mean(self.F_times)
    ...
    def SaveVis(self,vis=None,Col="CORRECTED_DATA",spw=0,DoPrint=True):
        ...
        if self.swapped:
            ...
            visout=vis
            # if self.flag_all is None:
            #     table_all = self.GiveMainTable()
            #     flag_all=table_all.getcol("FLAG")[:,self.ChanSlice,:]
            #     table_all.close()
            # else:
    ...
    def putcol(self,ColName,d,row0,nrows):
        ...
        log.print( "  Writing in %s:%s"%(self.MSName,ColName))
        t.putcol(ColName,d,row0,nrows)
        t.close()
