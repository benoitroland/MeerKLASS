...
class ClassImageSM():
    ...
    def givePreApplyDirs(self,iClusterDir):
        return self.DicoJonesDirToFacet[iClusterDir]["FacetsIDs"]
...
class ClassPreparePredict(ClassImagerDeconv):
    ...
    def LoadModel(self):
        ...
        if len(self.VS.ListMS)==1:
            model_freqs=self.VS.FreqBandChannelsDegrid[0]
            self.SM.MeanDataFreqPerSolBand=model_freqs
            self.SM.ChanMappingDegrid=self.VS.DicoMSChanMappingDegridding[0]
        else:
            f0=self.VS.GlobalFreqs.min()-self.VS.ListMS[0].ChanWidth[0]/2
            f1=self.VS.GlobalFreqs.max()+self.VS.ListMS[0].ChanWidth[0]/2
            if self.GD["GDkMS"]["ImageSkyModel"]["NChanPredictPerMS"]==0:
                NChanDeGrid=self.GD["GDkMS"]["Solvers"]["NChanSols"]
            else:
                NChanDeGrid=self.GD["GDkMS"]["ImageSkyModel"]["NChanPredictPerMS"]
                
            edges=np.linspace(f0,f1,NChanDeGrid+1)
            model_freqs=[]
            for iBin in range(NChanDeGrid):
                f0,f1=edges[iBin],edges[iBin+1]
                ff=set()
                for MS in self.VS.ListMS:
                    fff=MS.ChanFreq.ravel()
                    ind=np.where((fff>=f0)&(fff<f1))[0]
                    fff=fff[ind]
                    for ich in range(fff.size):
                        ff.add(fff[ich])
                model_freqs.append(np.mean(list(ff)))
            model_freqs=np.sort(np.array(model_freqs))
            self.SM.ChanMappingDegrid=np.argmin(np.abs(self.VS.GlobalFreqs.reshape((-1,1))-model_freqs.reshape((1,-1))),axis=1)

            NChanSols=self.GD["GDkMS"]["Solvers"]["NChanSols"]
            edges=np.linspace(f0,f1,NChanSols+1)
            freqsSolsBin=[]
            for iBin in range(NChanSols):
                f0,f1=edges[iBin],edges[iBin+1]
                ff=set()
                for MS in self.VS.ListMS:
                    fff=MS.ChanFreq.ravel()
                    ...
                    for ich in range(fff.size):
                        ff.add(fff[ich])
                freqsSolsBin.append(np.mean(list(ff)))
                
            freqsSolsBin=np.sort(np.array(freqsSolsBin))
            self.SM.MeanDataFreqPerSolBand=freqsSolsBin

        MS=self.VS.ListMS[0]
        OriginalRadec=MS.OriginalRadec
        lm_PhaseCenter=self.FacetMachine.CoordMachine.radec2lm(np.array([OriginalRadec[0]]), np.array([OriginalRadec[1]]))
        self.SM.lm_PhaseCenter=np.array(lm_PhaseCenter).ravel()
        ...
        ClusterCat=np.zeros((self.NDir,),dtype=ClassSM.dtypeClusterCat)
        ClusterCat=ClusterCat.view(np.recarray)
        ClusterCat.l=NodesCat.l
        ClusterCat.m=NodesCat.m
        ClusterCat.ra=NodesCat.ra
        ClusterCat.dec=NodesCat.dec
        for iCluster in range(ClusterCat.dec.size):
            setFacet=[]
            for i in range(len(self.DicoImager)):
                if self.DicoImager[i]["iSol"]==iCluster:
                    setFacet.append(str(i))
            StrF="/".join(setFacet)
            ClusterCat.Name[iCluster]="[S%i_F%s]" % (iCluster, StrF)
            ClusterCat.Cluster[iCluster]=iCluster
        NN=ClusterCat.shape[0]
        Cat=np.zeros((NN,),dtype=ClassSM.dtypeSourceList)
    ...
    def InitGrids(self):
        ...
        if "DeleteZeroValuedGrids" in list(inspect.signature(self.FacetMachine.set_model_grid).parameters.keys()):
            self.FacetMachine.set_model_grid(DeleteZeroValuedGrids=True)
        else:
        ...
        self.SM.NDirMain=self.SM.NDir
        self.SM.Dirs=self.Dirs
        log.print( "  There are %i non-zero directions in the image model"%self.SM.NDir)
        self.SM.ClusterCat=self.ClusterCat
        self.SM.SourceCat=self.SourceCat
    ...
    def PrepareGridMachinesMapping(self):
        ...
        for iFacet in sorted(self.FacetMachine.DicoImager.keys()):
            ...
            #print(iFacet,iDirJones,ClusterCat.SumI[iDirJones])
        ...
        for iDirJones in sorted(DicoJonesDirToFacet.keys()):
            ...
            if AppFlux[iDirJones]<=MaxAppFlux*Th:
                #log.print(ModColor.Str("  Remove Jones direction %3i [%f < %f Jy [Th = %f of Max %f Jy]]"%(iDirJones,AppFlux[iDirJones],MaxAppFlux*Th,Th,MaxAppFlux)))
                log.print("  Remove Jones direction %3i [%f < %f Jy [Th = %f of Max %f Jy]]"%(iDirJones,AppFlux[iDirJones],MaxAppFlux*Th,Th,MaxAppFlux))
                HasRemoved=1
                #print("  !!!!!!!!!!!!!!!!!!!!!!!")
            else:
        ...
        self.Dirs=list(self.DicoJonesDirToFacet.keys())
