...
def read_options():
    ...
    group.add_option('--iPol',type='int',help=' [no default]',default=0)
...
class PlotMachine():
    def __init__(self,options):
        ...
        for FileName in FilesList:
            ...
            log.print("%s solutions shape: %s"%(FileName,str(Sols.G.shape)))
        ...
        self.nt,self.nch,self.na,self.nd=nt,nch,na,nd
    ...
    def Plot(self,LSols,iDir=0):
        ...
        if self.options.iPol==0:
            xPol,yPol=0,0
        elif self.options.iPol==3:
            xPol,yPol=1,1
        else:
            stop
        ...
        if len(LSols)==1:
            Sols=LSols[0]
            ADir_0=op0(Sols.G[:,:,:,iDir,xPol,yPol])
            ADir_1=op1(Sols.G[:,:,:,iDir,xPol,yPol])
            vmin,vmax=0,2
            
        # print(np.angle(Sols.G[:,:,iDir,0,:]))

        if len(LSols)==2:
            Sols=LSols[0]
            ADir_0=op0(Sols.G[:,:,:,iDir,xPol,yPol])-op0(LSols[1].G[:,:,:,iDir,xPol,yPol])
            ADir_1=op1(Sols.G[:,:,:,iDir,xPol,yPol]*LSols[1].G[:,:,:,iDir,xPol,yPol].conj())
    ...
    # ######################"
    def PlotAllDir(self):
        for iChan in range(self.nch):
            self.PlotChan(self.LSols,iChan)
            
    def PlotChan(self,LSols,iChan=0):
        op0=np.abs
        op1=np.angle
        ...
        if len(LSols)==1:
            Sols=LSols[0]
            ADir_0=op0(Sols.G[:,iChan,:,:,xPol,yPol])
            ADir_1=op1(Sols.G[:,iChan,:,:,xPol,yPol])
            vmin,vmax=0,2
