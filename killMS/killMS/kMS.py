...
def read_options():
    ...
    OP.add_option('ConcatFreq',help='Concat frequencies [no default]')
    ...
    OP.add_option('AppendCalSource',type="str",help=' . Default is %default')

    OP.OptionGroup("* Sky FITS Restored Cube","FITSSkyModel")
    OP.add_option('FITSSkyModel',help='List of targets [no default]')
    OP.add_option('FITSNoiseBox',type=int,help=' [no default]')
    OP.add_option('FITSTh',type=float,help=' [no default]')


    OP.OptionGroup("* Sky DDFacet image related options","ImageSkyModel")
    ...
    OP.add_option('NChanPredictPerMS')
    ...
    OP.add_option('PhasedArrayMode',type="str",help='LOFAR beam mode. "AE" sets the beam model to Array and Element. Default is %default')
    ...
    OP.add_option('WeightFlagTh',type="float",help='Threshold of median bellow which the weight is set to zero. Default is %default')
    ...
    OP.add_option('UpdateWeights',help='Update imaging weights. Default is %default')
    ...
    OP.OptionGroup("* Smoother solver","Smoother")
    OP.add_option('SmoothEnable',type="str",help='Enable constrained solver. Default is %default')
    OP.add_option('SmoothInterpLevel',type="str",help='Smooth solution interpolation level. Default is %default')
    OP.add_option('SmoothInterpMode',type="str",help='Smooth solution interpolation mode. Default is %default')
    

    OP.OptionGroup("* Solver options","Solvers")
    OP.add_option('SolverType',help='Name of the solver to use (CohJones/KAFCA)')
    OP.add_option('AsyncPredict',help='Do the predict for the next slice asynchronously. Default is %default.',type="int")
    ...
    OP.add_option('Sequential',type="int",help='Sequentia/Asequential solver. Default is %default. ')
...
def main(OP=None,MSName=None):
    ...
    from killMS.Wirtinger.ClassWirtingerSolverLoops import ClassWirtingerSolverLoops as ClassWirtingerSolver
    ...
    NCPU = int(NCPU or psutil.cpu_count())
    ...
    GD=OP.DicoConfig
    CGiveSaveFileName=ClassGiveSolsFile.ClassGive_kMSFileName(options.MSName,GD=GD)
    ParsetName=CGiveSaveFileName.GiveFileName(Type="parset")
    ...
    AsyncProcessPool.init(ncpu=NCPU, affinity=0, num_io_processes=1)#GDPredict["Parallel"]["Affinity"],
                          #verbose=GDPredict["Debug"]["APPVerbose"])
    APP=AsyncProcessPool.APP
    
    if GD["Smoother"]["SmoothEnable"] is not None:
        from DDFacet.Other import AsyncProcessPool
        from DDFacet.Other import Multiprocessing
        AsyncProcessPool._init_default()
    ...
    if GD["SkyModel"]["SkyModel"]!="":
        ...
        PredictMode="Column"
    elif GD["FITSSkyModel"]["FITSSkyModel"] is not None:
        PredictMode="Catalog"
        FITSName=GD["FITSSkyModel"]["FITSSkyModel"]
        log.print(ModColor.Str("Predict Mode: using FITS file %s"%GD["FITSSkyModel"]["FITSSkyModel"]))
        from pyrap.tables import table
        tf=table("%s::SPECTRAL_WINDOW"%options.MSName,
                 ack=False)
        fMean=np.mean(tf.getcol("CHAN_FREQ"))
        tf.close()
        
        import SkyModel.PEX
        SM=SkyModel.PEX.mainFromExt(im=FITSName,
                                    NoiseBox=GD["FITSSkyModel"]["FITSNoiseBox"],
                                    snr=GD["FITSSkyModel"]["FITSTh"],
                                    ChFreq=fMean/1e6)
        SM.Cluster(NCluster=0,
                   DoPlot=0,
                   FromClusterCat=options.NodesFile)
        SM.MakeREG()
        SM.Save()
        options.SkyModel=GD["SkyModel"]["SkyModel"]=SM.NpFile
        ...
        #     GD["Beam"]['PhasedArrayMode']=options.PhasedArrayMode = GDPredict["Beam"]["PhasedArrayMode"]     # A/AE
        ...
        if GD["ImageSkyModel"]["NChanPredictPerMS"]==0:
            GDPredict["Freq"]["NDegridBand"]=options.NChanSols
        else:
            GDPredict["Freq"]["NDegridBand"]=GD["ImageSkyModel"]["NChanPredictPerMS"]
        ...
        # GDPredict["CF"]["OverS"]=21
        # GDPredict["CF"]["Support"]=15
        # GDPredict["Facets"]["Padding"]=1.3#2.5
        # GDPredict["Facets"]["FluxPaddingAppModel"]="image_ampphase1_di.app.model.fits"
        # GDPredict["Facets"]["FluxPaddingScale"]=4.
        
        if options.wmax is not None:
            GDPredict["CF"]["wmax"]=options.wmax

        # GDPredict["CF"]["wmax"]=100000.
        # GDPredict["CF"]["Nw"]=400
        ...
        FileDDFacet=options.MSName
        if options.ConcatFreq:
            MSName0 = options.MSName
            ListMSName = [ l.strip() for l in open(options.MSName).readlines() ]
            print("list file %s contains %d MSs" % (MSName0, len(ListMSName)), file=log)
            FileDDFacet=ListMSName
            
        VS_DDFacet=ClassVisServer_DDF.ClassVisServer(FileDDFacet,
    ...
    VS=ClassVisServer.ClassVisServer(options.MSName,
                                     ColName=ReadColName,
                                     TVisSizeMin=dt,
                                     DicoSelectOptions=DicoSelectOptions,
                                     TChunkSize=TChunk,
    ...
    CheckIn=ReadColName
    if "-" in CheckIn:
        CheckIn,_=CheckIn.split("-")
        
    if not(CheckIn in VS.MS.ColNames):
    ...
    if PredictMode=="Catalog":
        ...
        if GD["SkyModel"]["AppendCalSource"] is not None:
            ListBody=[]
            ListBodyIn=GD["SkyModel"]["AppendCalSource"]
            if isinstance(ListBodyIn, str): ListBodyIn=[ListBodyIn]
            if ListBodyIn[0].lower()=="all":
                ListBodyIn=["Sun","VirA","CasA","CygA","TauA"]
            for Body in ListBodyIn:
                FileName="Default"
                if ":" in Body:
                    ...
                    LBody=[Body]
                for Body in LBody:
                    ListBody.append({"Name":Body,"Time":VS.MS.tMean,"FileName":FileName})
        ...
        SM=ClassSM.ClassSM(options.SkyModel,
                           ...
                           invert=invert,
                           ListBody=ListBody,
                           radecCenter=(VS.MS.rarad,VS.MS.decrad))
        ...
        if GD["SkyModel"]["AppendCalSource"] is not None:
            ...
            SM2=ClassSM.ClassSM("Empty",
                                killdirs=kills,
                                invert=invert,
                                ListBody=ListBody,
                                radecCenter=(VS.MS.rarad,VS.MS.decrad))
            SM2.CutEmptyDirs()
            rac,decc=VS.MS.radec
            SM2.Calc_LM(rac,decc)
            SM2.Save()
            
            SM=ClassSM.ClassSMConcat([SM,SM2])
        ...
        SourceCat=np.zeros((1,),dtype=ClassSM.dtypeSourceList).view(np.recarray)
        SourceCat.ra=VS.MS.rac
        SourceCat.dec=VS.MS.decc
        SourceCat.Type=0
        SM=cSM(Type="Column",NDir=1,ClusterCat=ClusterCat,SourceCat=SourceCat)
    ...
    ConfigJacobianAntenna={"DoSmearing":DoSmearing,
                           ...
                           "VisToSolsChanMapping":VS.VisToSolsChanMapping}
    ...
    PM=ClassPredict(NCPU=NCPU,
                    ...
                    BeamAtFacet=(GD["Beam"]["BeamAt"].lower() == "facet"))
    PM2=None#ClassPredict_orig(NCPU=NCPU,IdMemShared=IdSharedMem)
    
    APP.registerJobHandlers(Solver)
    APP.registerJobHandlers(VS)
    if VS_PredictCol is not None:
        APP.registerJobHandlers(VS_PredictCol)
    
    APP.registerJobHandlers(PM)
    APP.startWorkers()
    APP.awaitWorkerStart()
    
    if SM.Type=="Image":
        PreparePredict.InitGrids()
        SM.MapClusterCatOrigToCut=PreparePredict.MapClusterCatOrigToCut
        SM.ClusterCatOrig=PreparePredict.ClusterCatOrig
        SM.NDirsOrig=PreparePredict.NDirsOrig
    elif SM.Type=="Hybrid":
        PreparePredict.InitGrids()
        SM.LSM[0].MapClusterCatOrigToCut=PreparePredict.MapClusterCatOrigToCut
        SM.LSM[0].ClusterCatOrig=PreparePredict.ClusterCatOrig
        SM.LSM[0].NDirsOrig=PreparePredict.NDirsOrig

        SM.ComputeMapping()
        
    Solver.InitSol(TestMode=False)
    Solver.setPredictParallel(PM)
    ...
    APP.shutdown()
    del(APP)
    #AsyncProcessPool._init_default(force=True)
    AsyncProcessPool.APP=None
    AsyncProcessPool.init(ncpu=NCPU, affinity=0, num_io_processes=1)#GDPredict["Parallel"]["Affinity"],
    APP=AsyncProcessPool.APP
    # if GD["Smoother"]["SmoothEnable"]:
    #     log.print(ModColor.Str("Will smooth solutions using %s mode"%GD["Smoother"]["SmoothInterpMode"],col="green"))
    #     CI=killMS.SmoothSols.ClassInterpol(OutSolsName=None,
    #                                        InterpMode=GD["Smoother"]["SmoothInterpMode"],
    #                                        DoPBar=False,#True,
    #                                        NCPU=options.NCPU,
    #                                        CrossMode=False)
    #     Solver.setSmoother(CI)
    #     APP.registerJobHandlers(CI)
    ...
    while True:
        ...
        if options.ExtSols=="":
            ...
            def SavePredict(ArrayName,FullPredictColName):
                ...
                DicoPredictedVis=shared_dict.attach("DicoPredictedVis_Chunk%i"%VS.iChunkThis)
                PredictData=DicoPredictedVis[ArrayName]
                
                if VS.MS.ToRADEC is not None:
                    PredictData=PredictData.copy()
                    DATA={"uvw":VS.MS.uvw,
                          "data":PredictData}
                    VS.MS.Rotate(DATA,RotateType=["vis"],Sense="ToPhaseCenter")
                    PredictData=DATA["data"]



                
                nrow_ThisChunk=VS.ThisDataChunk["ROW1"]-VS.ThisDataChunk["ROW0"]

                
                d=VS.MS.ToOrigFreqOrder(PredictData)

                VS.MS.putcol(FullPredictColName,d,VS.ThisDataChunk["ROW0"],nrow_ThisChunk)
            ...
            DoCompress=(Solver.SM_Compress is not None)
            ...
            if not DoCompress and (FreePredictGainColName!=None):
                ...
                if ":" in FullPredictColName: FullPredictColName,_=FullPredictColName.split(":")
            ...
            if not DoCompress and GD["SkyModel"]["FreeFullSub"]:
                if WriteColName is not None and WriteColName != "None" and WriteColName != "":
                    log.print( "Subtracting free predict from data")
                    DicoPredictedVis=shared_dict.attach("DicoPredictedVis_Chunk"%VS.iChunkThis)
                    PredictData=DicoPredictedVis["PredictedDataGains"]
                    Solver.VS.ThisDataChunk["data"]-=PredictData
                    log.print( "  save visibilities in %s column"%WriteColName)
                    d=VS.MS.ToOrigFreqOrder(VS.ThisDataChunk["data"])
                    VS.MS.putcol(WriteColName,d,VS.ThisDataChunk["ROW0"],VS.ThisDataChunk["ROW1"]-VS.ThisDataChunk["ROW0"])
            ...
            if SaveSols:
                ...
                def giveSolName(MSName):
                    CGiveSaveFileName=ClassGiveSolsFile.ClassGive_kMSFileName(MSName,GD=GD)
                    return CGiveSaveFileName.GiveFileName(Type="Sols")
                    # if options.SolsDir is None:
                ...
                SolFile=giveSolName(options.MSName)
                
                log.print( "Save Solutions in file: %s"%ModColor.Str(SolFile,col="green"))
                D=Solver.GiveDicoSols()
                np.savez(SolFile,**D)

                if options.ConcatFreq:
                    log.print('  Creating symbolic link in non-virtual MSs...')
                    for ThisMS in VS.MS.ListMS:
                        ThisMSName=ThisMS.MSName
                        symsolname=giveSolName(ThisMSName)
                        ...
                        os.symlink(os.path.abspath(SolFile),symsolname)
            ...
            FileName=CGiveSaveFileName.GiveFileName(ExtSolsName,Type="Sols")
            ...
            t0_ms=VS.MS.F_times.min()
            t1_ms=VS.MS.F_times.min()
            t0_sol=Sols.t0.min()
            t1_sol=Sols.t1.min()
            if t1_sol<t0_ms or t0_sol>t1_ms:
                log.print(ModColor.Str("Solution time range is outside the observation's"))
                log.print(ModColor.Str("   will compute the time median and greatly extend its time range"))
                Sols2=np.zeros((1,),dtype=Sols.dtype)
                Sols2["G"][0]=np.median(Sols["G"].real,axis=0)+1j*np.median(Sols["G"].imag,axis=0)
                Sols2["t0"][0]=0
                Sols2["t1"][0]=1e12
        ...
        if ((DoSubstract)|(DoApplyCal)|(ReWeight)):
            ...
            if options.BeamModel==None or not options.MergeBeamToAppliedSol:
                ...
                log.print("Merging kMS current solutions to PreApplyJones (including beam model)...")
            ...
            log.print("Compute VisToJonesMapping...")
            Map_VisToJones_Time,Map_VisToJones_Freq=DomainMachine.giveVisToJonesMapping(JonesMerged,times,freqs)
            Solver.VS.ThisDataChunk["Map_VisToJones_Time"]=Map_VisToJones_Time
            Solver.VS.ThisDataChunk["Map_VisToJones_Freq"]=Map_VisToJones_Freq
            ...
            if ("Resid" in options.ClipMethod) or ("DDEResid" in options.ClipMethod):
                ...
                if options.UpdateWeights:
                    ...
                    WAllChans=VS.MS.ToOrigFreqOrder(Weights)
                    VS.MS.putcol("IMAGING_WEIGHT",WAllChans,VS.ThisDataChunk["ROW0"],VS.ThisDataChunk["ROW1"]-VS.ThisDataChunk["ROW0"])
            ...
            if "ResidAnt" in options.ClipMethod and options.SubOnly==0:
                ...
                log.print("Compute VisToJonesMapping...")
                Map_VisToJones_Time,Map_VisToJones_Freq=DomainMachine.giveVisToJonesMapping(Jones,times,freqs)
                Solver.VS.ThisDataChunk["Map_VisToJones_Time"]=Map_VisToJones_Time
                Solver.VS.ThisDataChunk["Map_VisToJones_Freq"]=Map_VisToJones_Freq
                nrows=Solver.VS.ThisDataChunk["times"].size
                Solver.VS.ThisDataChunk["W"]=np.ones((nrows,Solver.VS.MS.ChanFreq.size),np.float64)
            ...
            if DoSubstract:
                ...
                indKills=None
                if options.SubOnly==0:
                    indKills=(SM.SourceCat.kill==1)
                    SM.SelectSubCat(SM.SourceCat.kill==1)
                    
                SourceCatSub=SM.SourceCat.copy()

                PredictData=PM.predictKernelPolCluster(Solver.VS.ThisDataChunk,Solver.SM,ApplyTimeJones=JonesMerged,indKills=indKills)
            ...
            if (DoSubstract|DoApplyCal):
                if WriteColName is not None and WriteColName != "None" and WriteColName != "":
                    ...
                    nrow_ThisChunk=VS.ThisDataChunk["ROW1"]-VS.ThisDataChunk["ROW0"]
                    d=VS.MS.ToOrigFreqOrder(VS.ThisDataChunk["data"])
                    VS.MS.putcol(WriteColName,d,VS.ThisDataChunk["ROW0"],nrow_ThisChunk)
                    #t.putcol("FLAG",VS.MS.ToOrigFreqOrder(Solver.VS.MS.flags_all),Solver.VS.MS.ROW0,Solver.VS.MS.ROW1-Solver.VS.MS.ROW0)
                else:
                    log.print("No output column specified. Skipping writing visibility data.")

    if APP is not None or GD["Smoother"]["SmoothInterpMode"]:
        #APP.terminate()
        APP=AsyncProcessPool.APP
        APP.shutdown()
        del(APP)
...
def GiveNoise(options,DicoSelectOptions,IdSharedMem,SM,PM,PM2,ConfigJacobianAntenna,GD):
    ...
    SolverInit=ClassWirtingerSolver(VSInit,SM,PolMode=options.PolMode,
                                    NIter=options.NIterLM,NCPU=NCPU,
                                    SolverType="CohJones",
                                    #DoPlot=options.DoPlot,
...
if __name__=="__main__":
    ...
    if ".txt" in MSName and not options.ConcatFreq:
        f=open(MSName)
        Ls=f.readlines()
        f.close()
        MSName=[]
        ...
        if type(lMS)==list:
            ...
            for MSName in lMS:
                if options.SkipExistingSols:
                    ...
                    #     FileName="%s/killMS.%s.sols.npz"%(DirName,SolsName)
                    CGiveSaveFileName=ClassGiveSolsFile.ClassGive_kMSFileName(MSName,GD=GD)
                    FileName=CGiveSaveFileName.GiveFileName(Type="Sols")
            ...
            ListSols=[]
            log.print("Merging solution files")
            try:
                for MSName in lMS:
                    CGiveSaveFileName=ClassGiveSolsFile.ClassGive_kMSFileName(MSName,GD=GD)
                    ListSols.append(CGiveSaveFileName.GiveFileName(Type="Sols"))
                
                CGiveSaveFileName=ClassGiveSolsFile.ClassGive_kMSFileName(options.MSName,GD=GD)
                OutFileName=CGiveSaveFileName.GiveFileName(Type="Sols")
                CM=killMS.MergeSols.ClassMergeSols(ListSols)
                CM.MakeDicoMerged()
                CM.Save(OutFileName)
            except Exception as e:
                log.print("Failed saving concatenated solution files with message: %s"%str(e))
