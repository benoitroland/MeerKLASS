...
def main(options=None):
    ...
    #SMName="ModelImage.txt.npy"
    #SMName="A-Team_4_Highres.short.mearby.skymodel.npy"
    SMName="Model.txt.npy"
    
    
    #ll=sorted(glob.glob("0000.p*.MS"))
    ll=sorted(glob.glob("nenutest.30min.ms"))
    #ll=sorted(glob.glob("0000.noW.MS"))
    #ll=sorted(glob.glob("L526161_SB339_uv.pre-cal_1288A51A4t_140MHz.pre-cal.ms.tsel"))
    #ll=sorted(glob.glob("20210917_000500_20210917_040000_NCP_COSMIC_DAWN_SB313-371.CAL.MS.tsel"))
...
class ClassSimul():
    ...
    def GiveSols(self):
        ...
        Amp_Mean=.9+np.random.rand(nch,na,nd)*1#*0.2*2
        ...
        period_Phase=300+np.random.randn(nch,na,nd)*1
        #period_Phase=np.random.randn(na,nd)*10
        PhaseAbs=np.random.randn(nch,na,nd)*np.pi#*0.3*2
        Amp_Phase=np.random.rand(nch,na,nd)*np.pi#*0.5*2
    ...
    def GiveJones(self):
        ...
        if self.BeamAt=="facet" and self.ApplyBeam:
        ...
        if ApplyBeam:
            ...
            if self.BeamMode=="LOFAR":
                ...
                GD={"Beam":{"Model":self.BeamMode,
                    ...
                    "DDESolutions":{"DDSols":None,
                                    "SolsDir":"SOLSDIR",
                                    "GlobalNorm":None,
                                    "JonesNormList":"AP"},
                    "Cache":{"Dir":""}
                }
                import DDFacet.Data.ClassMS as ClassMSDDF
                from DDFacet.Data import ClassJones
                ms=ClassMSDDF.ClassMS(MS.MSName,GD=GD,DoReadData=False,)
                ...
                # DicoDATA["flag"]=flag
                DicoDATA["times"]=self.MS.F_times
                # DicoDATA["A0"]=A0
                # DicoDATA["A1"]=A1
                # DicoDATA["u"]=u0
                # DicoDATA["v"]=v0
            ...
            if self.BeamMode=="LOFAR":
                def GB(time,ra,dec):
                    ...
                    for i in range(ra.shape[0]):
                        ...
                        # Beam.fill(0)
                        # Beam[...,0,0]=(time-self.MS.F_times[0])/3600#*(ra[i]+dec[i])+ra[i]+dec[i]
                        # Beam[...,1,1]=(time-self.MS.F_times[0])/3600#*(ra[i]+dec[i])+ra[i]+dec[i]
                    return Beam
            else:
                def GB(time,ra,dec):
                    # DicoBeamDDF=DicoDATA["Beam"]
                    # tm=(DicoBeamDDF["Jones"]["t0"]+DicoBeam["Jones"]["t1"])/2
                    # iTime=np.argmin(np.abs(time-tm))
                    # print(iTime)
                    # nd,na,nch,2,2
                    
                    BB=JonesMachine.GiveInstrumentBeam(time,ra,dec)

                    return BB
                
            for itime in range(Tm.size):
                print("Beam %i/%i"%(itime,Tm.size))
    ...
    def DoSimul(self):
        ...
        Load=VS._workerLoadNextVisChunk(0)
        ...
        #PredictData=PM5.predictKernelPolCluster(VS.ThisDataChunk,
        #                                        SM,
        #                                        ApplyTimeJones=Jones,Noise=Noise)
