...
log=logger.getLogger("ClassMergeSols")
...
class ClassMergeSols():
    def __init__(self,ListFilesSols):
        self.ListFilesSols=[os.path.abspath(f) for f in ListFilesSols]
    ...
    def CheckConformity(self):
        ...
        for iSol in range(1,self.NSolsFile):
            ...
            C3=np.allclose(self.ListDictSols[0]["PreApplyTimes"]["t0"],self.ListDictSols[0]["PreApplyTimes"]["t0"])
            C4=np.allclose(self.ListDictSols[0]["PreApplyTimes"]["t1"],self.ListDictSols[0]["PreApplyTimes"]["t1"])
            C5=np.allclose(self.ListDictSols[0]["BeamTimes"]["t0"],self.ListDictSols[0]["BeamTimes"]["t0"])
            C6=np.allclose(self.ListDictSols[0]["BeamTimes"]["t1"],self.ListDictSols[0]["BeamTimes"]["t1"])
            #C4=np.allclose(self.ListDictSols[0]["ClusterCat"],self.ListDictSols[iSol]["ClusterCat"])
            if not (C0 and C1 and C3 and  C4 and C5 and C6):
    ...
    def MakeDicoMerged(self):
        ...
        DicoOut['SourceCatSub']=Dico0.get('SourceCatSub',None)
        ...
        ArraySolsPath=np.zeros((NFreqsOut,),"|S200")
        for iSol in range(self.NSolsFile):
            ...
            ArraySolsPath[iFreq]=self.ListFilesSols[iSol]
        ...
        DicoOut['ArraySolsPath']=ArraySolsPath
    ...
    def Save(self,FileOut):
        ...
        log.print(ModColor.Str("  Saving interpolated solutions in: %s"%FileOut,col="green"))
