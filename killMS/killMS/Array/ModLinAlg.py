...
def invSVD(A):
    ...
    try:
        u,s,v=np.linalg.svd(np.complex128(A))#+np.random.randn(*A.shape)*1e-10*A.max())
        #u,s,v=np.linalg.svd(np.complex128(A))#+np.random.randn(*A.shape)*1e-10*A.max())
    except:
...
def getErr(A,Cut=5.):
    Ai=invSVD2(A,Cut=Cut)
    NTry=100
    x=np.random.randn(A.shape[0],NTry)+1j*np.random.randn(A.shape[0],NTry)
    y=Ai.dot(A).dot(x)
    return np.var(y-x,axis=1)


def invSVD2(A,Cut=1.6):


    a=A
    #a+=np.random.randn(*a.shape)*a.max()*1e-10
    u, s, vh = np.linalg.svd(np.complex128(a))#, full_matrices=True)
    # print("u=",u)
    # print()
    # print("s=",s)
    # print()
    ...
    sa=np.abs(s)
    m,n=A.shape
    #k=np.count_nonzero(sa>sa.max()*1e-6)
    k=np.count_nonzero(sa>sa.max()/ 10**Cut)
    k=np.min([k,u.shape[1],vh.shape[0]])
    # print("lllllllll",k)
    u=u[:,0:k]
    vh=vh[0:k,:]
    s=s[0:k]
    nx,ny=u.shape[0],vh.shape[1]
    
    
    
    #smat[:nm, :nm] = np.diag(1./s)
    #smat[:n1, :n1] = np.diag(s[0:n1])

    # smat = np.diag(s)
    # print(A-np.dot(u, np.dot(smat, vh)))

    
    smat = np.diag(1/s)
    Ainv=np.dot(vh.T.conj(), np.dot(smat.T.conj(), u.T.conj()))
    # print(np.dot(Ainv,A))
    
    return Ainv# np.dot(u, np.dot(smat, vh))
