...
class ClassFitAmp_1d():
    def __init__(self,gains,nu,
                 Order=2,
                 Tol=5e-2,Incr=1,RemoveMedianAmp=True,LogMode=False):
        Order=np.min([nu.size-3,Order])
        #print("Order",Order)
        self.Order=Order
        self.nf=gains.size
        self.RemoveMedianAmp=RemoveMedianAmp
        self.G=gains.copy().reshape((self.nf,))
    ...
    def doSmooth(self):
        vmax=2.
        vmin=.3
        mout=np.zeros((self.nf,),np.float32)
        ii=0
        while True:
            self.GOut[:]=1.
            x=np.arange(self.nf)
            ...
            yc[ind]=y[ind]
            xc=x[ind]
            z=np.polyfit(x[ind], y[ind], self.Order)
            #print("zzzzz",self.Order,z.size)
            P = np.poly1d(z)#,r=False)
        ...
        #     off=np.median(self.G[:]-self.GOut[:],axis=0)
        #     self.GOut[:]=self.GOut[:]+off
        # if not self.LogMode:
...
class ClassFitAmp():
    def __init__(self,gains,nu,Tol=5e-2,Incr=1,RemoveMedianAmp=True,LogMode=False,iDir=None):
    ...
    def doSmooth(self):
        ...
        vmin=.3
        for iAnt in range(self.na):#[::-1]:
            MaskOnes=np.float32(self.G[:,:,iAnt]==1)
            # BoxTime=5
            # BoxFreq=5
            # MaskOnes=(scipy.signal.convolve2d(MaskOnes,np.ones((BoxFreq,BoxTime),np.float32),mode="same")!=0)

            for iChan in range(self.nf):
                # print
                # print "ichan %i %i (%i,%i)"%(iChan,iAnt,self.nf,self.na)
                indOnes=np.where(MaskOnes[:,iChan]==1)[0]
                ...
                while True:
                    ...
                    m=np.ones_like(y)
                    m[1::]=m0[:]
                    ind=np.where((np.abs(y)!=0) & (m!=0) & (np.isnan(y)==0) & (mout==0))[0]
                    # print np.count_nonzero(ind)
                    yc=np.ones_like(y)
                    if ind.size<2:
                        break
                    yc[ind]=y[ind]
                    xc=x[ind]
                    #print(iChan,self.iDir,iAnt)
                    #print(    ind)
                    z=np.polyfit(x[ind], y[ind], np.min([ind.size-1,10]) )
                    p = np.poly1d(z)
                    px=np.abs(p(x))
                    pp=px#/np.mean(px)*np.median(y[ind])
                    self.GOut[:,iChan,iAnt]=pp

                    # if iChan==17 and self.iDir==19 and iAnt==19:
                    #     import pylab
                    #     op=np.abs
                    #     pylab.clf()
                    #     pylab.scatter(x, y,color="black")
                    #     pylab.scatter(x[ind], y[ind],color="red")
                    #     pylab.plot(px,color="black")
                    #     pylab.plot(pp,color="red")
                    #     pylab.draw()
                    #     pylab.show(block=False)
                    #     pylab.pause(0.1)
                    #     #stop
                ...
                if ind.size<2:
                    continue
                if indOnes.size>0:
                    self.GOut[indOnes,iChan,iAnt]=1.
