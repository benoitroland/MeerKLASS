    ...
    def doSmooth(self):
        ...
        for iAnt in range(self.na):#[::-1]:
            for iTime in range(self.nt):
                ...
                ii=0
                while True:
                    self.GOut[iTime,:,iAnt]=1.
                    x=np.arange(self.nt)
                    y=self.G[iTime,:,iAnt].copy()
                    ...
                    yc[ind]=y[ind]
                    xc=x[ind]
                    z=np.polyfit(x[ind], y[ind], 2)
                    p = np.poly1d(z)
                    px=np.abs(p(x))
                    pp=px/np.mean(px)*np.median(y[ind])
                    self.GOut[iTime,:,iAnt]=pp
                    ...
                    if s_mout0==s_mout1:
                        self.GOut[iTime,:,iAnt]=np.abs(p(x))
                        # print iChan,ii,s_mout0
                        break
                    ii+=1
                    ## print ii
                Ag=np.abs(self.GOut[iTime,:,iAnt])
