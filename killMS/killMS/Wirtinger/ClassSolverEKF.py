...
class ClassSolverEKF(ClassJacobianAntenna):
    ...
    def GivePaPol(self,Pa_in,ipol,CorrConditioning=True):
        PaPol=Pa_in.reshape((self.NDir,self.NJacobBlocks_X,self.NJacobBlocks_Y,self.NDir,self.NJacobBlocks_X,self.NJacobBlocks_Y)).copy()

        # if CorrConditioning:
        #     for ipol in range(self.NJacobBlocks_X):
        #         JHJ=self.L_JHJ[ipol]#*(1./rms**2)
        #         Err=ModLinAlg.getErr(JHJ)#*5+1)
        #         for iDir in range(self.NDir):
        #             PaPol[iDir,ipol,:,iDir,ipol,:]+=Err[iDir]
    ...
    def PrepareJHJ_EKF(self,Pa_in,rms):
        ...
        for ipol in range(self.NJacobBlocks_X):
            ...
            # # ##############################################
            # nDir,nr,nch=self.K_XX.shape
            # K=self.K_XX.copy()
            # K=K.reshape((nDir,nr*nch))
            # KK=np.dot(K,K.T.conj())
            # u,s,v=np.linalg.svd(KK)
            # # print(KK.shape)
            # indf=(self.DicoData["flags_flat"]==1)
            # fflagged=np.count_nonzero(indf)/indf.size
            # np.savez("KMS_DEBUG/JHJ_%02i_%04i_%02i_%i.npz"%(self.iAnt,self.iiCount,self.LMIter,ipol),
            #          JHJ=JHJ,
            #          fflagged=np.array([fflagged],np.float32),
            #          s=s.ravel(),
            #          KK=KK,
            #          #Ga=self.Ga,
            #          JHJinv=JHJinv,
            #          PaPol=PaPol,
            #          Pinv=Pinv)
            # np.save("KMS_DEBUG/JHJ_%02i_%04i_%02i_%i.Ga.npy"%(self.iAnt,self.iiCount,self.LMIter,ipol),self.Ga)
            # # ##############################################
    ...
    def giveErrAntDir(self):
        # JHJ=self.L_JHJ[0]
        # Err=ModLinAlg.getErr(JHJ)
        return self.Err
    ...
    def doEKFStep(self,Gains,P,evP,rms,Gains0Iter=None):
        ...
        # if self.iAnt==21: stop
        
        Pa_new1=np.diag(np.diag(Pa_new1))
...
#            self.CalcKernelMatrix(rms)
        ...
        # JHJ=self.L_JHJ[0]
        # print(ModLinAlg.getErr(JHJ))
        # Err=ModLinAlg.getErr(JHJ)#*5
        # for iDir in range(self.NDir):
        #     Pa[iDir,iDir]+=Err[iDir]
        #     print(iDir,Err[iDir])
        ...
        self.Ga=self.GiveSubVecGainAnt(Gains)
        ...
        evPa=np.diag(evPa).copy()
        #evPa[np.abs(evPa)>1]=1.
        evPa=np.diag(evPa)
