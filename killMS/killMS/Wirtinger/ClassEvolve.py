...
class ClassModelEvolution():
    ...
    def Evolve0(self,Gin,Pa,kapa=1.):
        DicoSolsArray=shared_dict.attach("DicoSolsArray")
        done=DicoSolsArray["done"]
        
        indDone=np.where(done==1)[0]
        # if self.iAnt==23:
        #     print("lkjhlfkgjh",self.iAnt,kapa,np.max(Pa),np.max(Gin))
        #print type(NpShared.GiveArray("%sSharedCovariance_Q"%self.IdSharedMem))
        DicoCov=shared_dict.attach("DicoCov")
        Q=kapa*DicoCov["Q"][self.iChanSol,self.iAnt]
        ...
        G=DicoSolsArray["G"][indDone][:,self.iChanSol,self.iAnt,:,0,0]
        Gm=np.mean(G,axis=-1)
        dG=Gm[:-1]-Gm[1:]
        done0=done#NpShared.GiveArray("%sSolsArray_done"%self.IdSharedMem)
        ...
        t0=DicoSolsArray["t0"][indDone]#NpShared.GiveArray("%sSolsArray_t0"%self.IdSharedMem)[indDone]
        t1=DicoSolsArray["t1"][indDone]#NpShared.GiveArray("%sSolsArray_t1"%self.IdSharedMem)[indDone]
        tm=DicoSolsArray["tm"][indDone]#NpShared.GiveArray("%sSolsArray_tm"%self.IdSharedMem)[indDone]


        G=DicoSolsArray["G"][indDone][:,self.iChanSol,self.iAnt,:,:,:] #NpShared.GiveArray("%sSolsArray_G"%self.IdSharedMem)[indDone][:,self.iChanSol,self.iAnt,:,:,:]
