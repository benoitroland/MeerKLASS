...
{
  PyObject *ObjVisIn;
  PyObject *LSM, *LUVWSpeed, *LFreqs,*LSmearMode, *LJones, *LExp,*LSinc,*LBessel;
  PyArrayObject *NpVisIn, *NpUVWin, *matout;
  ...
  if (!PyArg_ParseTuple(args, "O!O!O!O!O!O!iO!O!O!",
			//&ObjVisIn,
			&PyArray_Type, &NpVisIn, 
			&PyArray_Type, &NpUVWin, 
			&PyList_Type, &LFreqs,
  ...
  //===================
  // Bessel table: sin(x)/x
  PyArrayObject *Np_Bessel;
  Np_Bessel = (PyArrayObject *) (PyList_GetItem(LBessel, 0));
  float *p_Bessel=(float *) PyArray_DATA(Np_Bessel);
  npy_intp *Np_Bessel_DIMS=PyArray_DIMS(Np_Bessel);
  int NmaxBessel=Np_Bessel_DIMS[0];
  PyObject *_FStepBessel= PyList_GetItem(LBessel, 1);
  float StepBessel=(float) (PyFloat_AsDouble(_FStepBessel));
  //printf("!!! %f %f\n",NmaxBessel,StepBessel);
  //===================
  ...
  for(dd=0;dd<ndir;dd++){
    ...
    UVWin=p1;
    float ang=p_GPA[dd];
    int SType=p_SType[dd];
    
    float SigMaj=p_Gmaj[dd];
    float SigMin=p_Gmin[dd];
    
    /* printf("%i:\n",dd); */
    /* printf("%f %f %f %f\n",SigMin,SigMaj,ang,SType); */

    ThisSourceType=SType;//(SigMaj!=0.);
    ...
    //for ( irow=1997; irow<1999; irow++)  {
      ...
      if(ThisSourceType==1){
    	...
    	UVsq=(G0*G0+G1*G1);
	//printf("UVsq %f %f %f\n",G0,G1,UVsq);
      } else if(ThisSourceType==2){
    	UVsq=sqrtf((float)(U*U+V*V));
      } 
	  ...
	  //printf("ThisSourceType %i\n",(int)ThisSourceType);
    	  if(ThisSourceType==1){
    	    UVsq_ch=c2[ch]*UVsq;
    	    FGauss=p_Flux[dd*nchan+ch]*GiveFunc(UVsq_ch,p_Exp,StepExp, NmaxExp);
	    //printf("FGauss %f\n",FGauss);
    	  } else if(ThisSourceType==2){
	    //phi=PI/2.*UVsq/(WaveL[ch])*SigMaj;///0.85;

	    /* phi=PI*UVsq/(WaveL[ch])*SigMaj;///0.85; */
	    /* /\* //printf("ch=%i Wl=%f\n",ch,(float)Wl); *\/ */
	    /* if(phi==0){ */
	    /*   phi=0.0001; */
	    /* }; */
	    ...
	    /* float phiU=PI*U/(WaveL[ch])*SigMaj;///0.85; */
	    /* float phiV=PI*V/(WaveL[ch])*SigMaj;///0.85; */
	    /* //printf("ch=%i Wl=%f\n",ch,(float)Wl); */
	    /* if(phiU==0){ */
	    /*   phiU=0.0001; */
	    /* }; */
	    ...
	    /* decorr=(float)sin((double)phiU)/((double)phiU) * (float)sin((double)phiV)/((double)phiV); */
	    //decorr=1.;
	    ...
	    decorr=GiveFunc(fabs(phi),p_Bessel,StepBessel, NmaxBessel);
	    
    	    FGauss=p_Flux[dd*nchan+ch]*decorr;//GiveFunc(fabs(phi),p_Sinc,StepSinc, NmaxSinc);
	    //printf("FGauss %f %f %f\n",FGauss,phi,UVsq);
	  }
    	  else{
	  ...
	  //printf("FGauss %f\n",FGauss);
	      ...
	      //printf("[%i, %f, %f]f, %f %f %f\n",ch,(float)p_DFreqs[ch],phase,phi,decorr,decorr2);
    	      result*=decorr;
    	    };
    	  };
